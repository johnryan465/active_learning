...
class BayesianModule(Module):
    ...
    To be efficient, the module allows for a part of the forward pass to be
    deterministic.
    """
...
class _ConsistentMCDropout(Module):
    ...
    def _create_mask(self, input, k):
        mask_shape = [1, k] + list(
            self._get_sample_mask_shape(input.shape[1:])
        )
        mask = torch.empty(mask_shape, dtype=torch.bool,
                           device=input.device).bernoulli_(self.p)
        return mask
